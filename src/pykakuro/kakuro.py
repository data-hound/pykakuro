...
class Solution(object):
  ...
  def get_lol(self, **kwargs):
    "Generates a list of list of the solution"
    return print_lol(self.data, self.puzzle.x_size, **kwargs)
...
class Kakuro(object):
  ...
  Board Layout
  ============
  Sum Hints:          (A,D): A - sum across, D - sum down
  Black cells:        0
  Fillable Cells:     1
  """
  def __str__(self):
  ...
  def solve(self, timeout=None, timeout_exception=True):
    ...
    try:
      ...
      # will get stuck.
      self.brute_force_size = BRUTE_FORCE_WARN_LIMIT
      self.difficulty = 0.05 * math.log(BRUTE_FORCE_WARN_LIMIT) + 0.01 * self.num_entry_squares 
  ...
  def get_lol(self, **kwargs):
    "Generates a list of list of the solution"
    return print_lol(self.data, self.x_size, **kwargs)
  ...
  def unsolve(self):
    ...
    intact."""
    self.is_solved = False
    self.master_solution = self.data.copy()
  ...
  def check_solutions(self):
    for solution in self.solutions:
      self.check_solution(solution.data)
  
  def check_all_solved(self, data):
    """
      Check if all cells filled in the solution
    """
    solution_list = [*data]
    puzzle_list = self.data

    unfilled_idxs = []
    for idx,(el1, el2) in enumerate(zip(puzzle_list, solution_list)):
      if el1 == 1 and el2 == 0:
        unfilled_idxs.append(idx)
    
    if unfilled_idxs:
      return False, unfilled_idxs
    else:
      return True, unfilled_idxs
  ...
  def check_solution(self, data):
    ...
    all_solved, unfilled = self.check_all_solved(data)

    if not all_solved:
      raise SolutionInvalidException(f"Unfilled entries found at {unfilled}")
...
def get_by_row(data, x_size, center_cells = True):
  strings = []
  for x in data:
    if type(x) == type(()):
      strings.append( 
...
def print_lol(data, x_size, center_cells=True):
  """
    Print the puzzle string in a list-of-lists style
  """

  by_row = get_by_row(data, x_size, center_cells)
  string_lol = ''
...
def gen_random(x_size=10, y_size=10, is_solved=True, is_exclusive=True,
  ...
  for idx in range(x_size*y_size):
    ...
    if random.random() > 0.6:
      ...
      for _ in range(20):
        ...
        if is_exclusive:
          if (val not in row(arr, x_size, row_idx) and
              val not in col(arr, x_size, col_idx)):
            arr[idx] = val
            break
        else:
          arr[idx] = val
  ...
  # 0-out top and left
  arr[0:x_size] = (0,)*x_size
  arr[0::x_size] = (0,)*y_size
  ...
  for x in range(0, x_size):
    for i in range(len(arr) - x_size + x, -1, -x_size):
      if arr[i] and type(arr[i]) != type(()):
        sum += arr[i]
      elif sum:
        if type(arr[i]) == type(()):
          arr[i] = arr[i][0], sum
  ...
  k=Kakuro(x_size, arr, min_val, max_val, is_exclusive)
  if not is_solved:
    k.unsolve()
...
def _process_row_or_col(record, row_or_col, is_entry_square):
  ...
  while record:
    ...
    if not is_entry_square(cell):
      raise ConstraintWithoutEntryCellException(orig_record)
    cells = [cell]
    while record:
      cell = record.pop()
      if not is_entry_square(cell):
...
def get_vals(sum_val, n):
  ...
  ((1, 2, 4))
  """
  return tuple(x for x in combinations(list(range(1, sum_val)),n) if
...
def _generate_set_cache():
  ...
  with open('.set_cache', 'wb') as f:
    pickle.dump(get_set_cache, f, pickle.HIGHEST_PROTOCOL)
...
def _prune_by_count(cells):
  ...
  for src_cells_set, count in list(c.items()):
